 ...
 */
@DisplayName("원장 소유자 배선 (notification-service)")
class LedgerOwnerWiringTest {
    ...
    @Test
    @DisplayName("컨테이너에 올린 LedgerOwner 는 NOTIFICATION 다 — DeadLetterConsumer.SELF 와 같아야 한다")
    void ledgerOwnerIsThisService() {
        runner.run(context -> {
            ...
            assertThat(owner.service()).isEqualTo(PeekcartService.NOTIFICATION);
            assertThat(owner.prefix()).isEqualTo("notification");
            assertThat(owner.owns("notification")).isTrue();
