package com.peekcart.notification.observability;
...
/**
 * notification-service 관측성 계약 회귀 (ADR-0009 §45-48 서비스별 복제).
...
@DisplayName("notification-service 관측성 계약 회귀 테스트")
class NotificationObservabilityMetricsIntegrationTest {
    ...
    @Test
    @DisplayName("/actuator/prometheus 에 application=notification-service 태그와 histogram bucket 이 노출된다")
    void prometheus_exposesApplicationTagAndHistogramBucket() {
        // 요청 메트릭 생성 (health 호출 — permitAll)
        ...
        assertThat(body)
                ...
                .contains("application=\"notification-service\"");
        assertThat(body)
    ...
    void actuatorExposure_whitelistsHealthAndPrometheus() {
        ...
        assertThat(restTemplate.getForEntity("/actuator/info", String.class).getStatusCode())
                .as("info 는 미노출 (exposure include + SecurityConfig 이중 보호)")
