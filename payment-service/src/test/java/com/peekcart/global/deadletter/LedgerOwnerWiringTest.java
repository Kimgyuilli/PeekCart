 ...
 */
@DisplayName("원장 소유자 배선 (payment-service)")
class LedgerOwnerWiringTest {
    ...
    @Test
    @DisplayName("컨테이너에 올린 LedgerOwner 는 PAYMENT 다 — DeadLetterConsumer.SELF 와 같아야 한다")
    void ledgerOwnerIsThisService() {
        runner.run(context -> {
            ...
            assertThat(owner.service()).isEqualTo(PeekcartService.PAYMENT);
            assertThat(owner.prefix()).isEqualTo("payment");
            assertThat(owner.owns("payment")).isTrue();
