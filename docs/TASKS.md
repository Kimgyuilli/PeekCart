...
| ② | 서비스별 DB 분리 (PR1 ✅ [#69](https://github.com/Kimgyuilli/PeakCart/pull/69) 교차 도메인 FK 6개 드롭(`fk_carts_user`·`fk_cart_items_product`·`fk_orders_user`·`fk_order_items_product`·`fk_payments_order`·`fk_notifications_user`, 컬럼 유지·ID 참조 대체)·소유 경계 검증(5서비스 자기 테이블만 매핑)·8모듈 그린, ADR-0012 D1 — 물리 분리 선행 저위험 단계 · 계획서 `task-impl2-db-per-service.md` PR1/PR2/PR3 분할) · **다음 PR2** Flyway per-service 모듈화+물리 스키마 분리(1 인스턴스+5 스키마)+k8s/CI · **PR3** retention 스케줄러(D5) | A3 | L-008/011 | 🔄 |
