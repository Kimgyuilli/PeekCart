...
| 이미지 repo 계약 정렬 | CI/K8s/문서 image repository 이름 단일화 (+ 가능하면 CI lint) | 도식검토 | **D-015** | 🔲 | — |
| Grafana→Slack 경로 | Slack contact point provisioning 또는 범위 정정 | 도식검토 | **D-017** | 🔲 | — |
...
| D-015 | Deploy/CI | CI push image repo(`peekcart`) ↔ K8s base/GKE 참조(`peakcart`) 계약 불일치 → GHCR→AR 복사·base 배포 실패 가능 | 버킷 1 (도식검토) | 🔲 대기 |
| D-017 | Observability | Grafana alert rule 존재하나 Slack contact point/provisioning 부재 → "alert→Slack" 경로 미완성 | 버킷 1 (도식검토) | 🔲 대기 |
