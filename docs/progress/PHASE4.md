...
> 설계 ADR(A1~A4, #44~#47)이 SSOT. 각 구현 항목은 선행 ADR을 따라 PR 단위로 진행하며, 세부 PR 분할은 `/plan` 착수 시 정의한다.

### (착수 예정) ① Gradle 멀티모듈 전환 — 선행 ADR-0011
...
- ⚠️ 대규모 리팩토링 — work diff 大, 실제 빌드/테스트 동반. PR 분할 권장(common 추출 → 서비스 순차)
