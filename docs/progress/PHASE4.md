...
## PR 본문 문체 재작성 — 이미 끝나 있던 작업과 복원 경로 (2026-09-19)

### 대기 중이라던 작업이 실물에서는 완료였다

`task-pr-body-reformat` 은 체크박스 0/7 에 TASKS 도 대기였다. P1(백업 무결성 확인)을
하다가 라이브 PR 이 이미 재작성 상태인 것을 발견했다. 이전 세션이 작업은 했는데 기록을
남기지 않았다.

사후 검증으로 충족을 확인했다. 라이브 PR 125개 전부 `writing-lint --file` 오류 0건,
스냅샷 원문 124개(대조군)는 121개 PR 에서 검출. 링크·`#번호`·부채/ADR ID 소실 0,
신규 사실 0, 코드펜스 바이트 불일치 0. 신규 숫자 4건은 전부 `구현 ①` 을 `구현 1` 로
푼 것이라 규약이 요구한 변환이다.

대조군을 함께 돌린 이유는 라이브 0건만으로는 lint 가 아무것도 안 보는 경우와 구분되지
않기 때문이다.

### 복원 경로를 뒤늦게 만들었다

`scripts/pr-body-restore.sh`. 계획서가 적은 `.cache/pr-backup-latest` 는 없었다. `.cache`
는 git 밖이라 세션 사이에 사라졌고, 남은 원문은 커밋된
`docs/progress/evidence/pr-bodies-snapshot-20260919.json` 뿐이다. D-028 때 이 스냅샷을
`plans-archive.sh` 가 재작성했다가 되돌린 적이 있는데, 그때 보존을 우선한 판단이
여기서 값을 했다. 그 파일이 사라졌으면 복원 경로 자체가 없었다.

되돌릴 수 없는 상태로 124개를 고치지 않는다는 것이 계획서 전제였는데, 실제로는 **고친
뒤에** 복원 경로가 생겼다. 순서가 뒤집혔다.

### 실물 왕복이 결함 두 개를 잡았다

PR #1 의 현재 본문을 임시 스냅샷으로 뜨고 망가뜨린 뒤 복원해 바이트를 비교했다.

| 결함 | 증상 | 원인 |
|---|---|---|
| 끝 개행 손실 | 복원본이 1바이트 짧다 (1220 대 1219) | `$(...)` 가 끝 개행을 잘라낸다 |
| 파이프 미도달 | self-test 가 빈 JSON 을 받는다 | heredoc 이 python 의 stdin 을 차지한다 |

두 번째는 `writing-lint.sh` 가 주석으로 남겨 둔 함정을 그대로 다시 밟은 것이다. 같은 레포
안에 경고가 적혀 있어도 다시 밟힌다.

본문을 셸 변수에 담지 않고 파일로만 주고받도록 고친 뒤 재복원해 제목·본문 바이트 동일을
확인했다 (sha256 `e89063106ce1e561`). PR #1 은 검증 전 상태로 돌아갔고 순변화는 없다.

### 검증

self-test 9건 통과. 변이 시험으로 바이트 비교가 실제로 결함을 잡는지 확인했다. 본문을
다시 `$(...)` 로 나르게 되돌리자 9바이트 대 14바이트로 red 가 됐다.

### 미충족

- **커밋 메시지의 em dash 는 그대로다.** 범위 밖으로 결정된 사항이다
- **이전 세션이 왜 기록을 안 남겼는지는 모른다.** 재구성할 근거가 없다
