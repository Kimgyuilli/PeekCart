...
### 검증이 확대한 것 — 진짜 결함은 "과금 잔존"이 아니었다

착수 전 코드 검증 C-1~C-10 중 **C-3 이 문제 정의를 바꿨다.** 같은 트랜잭션 안에
`assignPaymentKey` 가 있다. 롤백하면 `payments.payment_key` 가 **생성 시 UUID placeholder 로
되돌아간다** — 그 UUID 로는 Toss 조회가 불가능하다. 즉 과금이 남는 것보다 나쁜 것은
**사후에 과금 여부를 확인할 수단 자체가 사라진다**는 것이다.

이 확대가 Alternative A(트랜잭션만 떼고 호출 순서 조정)를 기각시켰다. 순서를 바꿔도
`paymentKey` 를 커밋할 시점이 PG 호출 *이후*뿐이라 같은 구멍이 남는다. **T1 이 키를 커밋해야
한다**는 것이 설계의 본체가 됐다.

또 **C-7 은 계획에 유리한 쪽으로 반증됐다** — 고아 과금을 환불로 보낼 때
`PaymentRefundService.succeed()` 를 고쳐야 할 줄 알았으나, 이미 `payments` 상태를 확인하고
`APPROVED` 일 때만 `markRefunded()` 하고 있었다. 우회 진입점 하나만 추가하면 됐다.

### 같은 문제를 이미 푼 표면이 있었다

ADR-0018 D3 이 환불 경로에서 **동일한 crash 경계**를 풀어둔 상태였다 — fence 원장 +
claim(T1)/PG(트랜잭션 밖)/확정(T2) + fencing token + 조회 기반 reconciliation. 승인 경로만
그 계약 밖에 남아 있었다. 새 모델을 발명하는 대신 같은 형태로 맞췄다. 운영자가 배워야 할
모델이 둘이 아니라 하나다.

### 같게 하지 않은 것 셋 — 여기가 결정이다

- **`REQUESTED` 상태를 두지 않았다.** 환불은 비동기 트리거라 "요청은 커밋됐고 실행자는 아직
  안 집었다" 는 구간이 실재하지만, 승인은 사용자 요청이 곧 실행 시작이라 그 구간이 없다.
  만들면 도달하지 않는 상태가 된다.
- **재시도 루프를 넣지 않았다(`max-attempts` 개념 자체가 없다).** 승인은 사용자가 기다리는
  동기 요청이다. 재시도하면 응답이 시도 수에 비례해 늘고 그동안 예약 lease 가 만료된다
  (`leaseApprovalMargin` 의 전제가 깨진다). 일시 실패는 `UNRESOLVED` 로 넘겨 **재시도의
  소유자를 배경 잡으로 옮겼다.**
- **reconciliation 이 승인을 재호출하지 않는다.** 환불의 `verifyThenExecute` 는 "취소된 적
  없음"이 확정되면 같은 멱등키로 재호출하지만, 승인 재호출은 **없던 과금을 새로 만드는
  행위**다. 그 시점엔 사용자 세션도 lease 도 없다. 조회로 확정만 한다.

같은 이유로 `claim-lease` 를 환불처럼 부팅 불변식으로 강제하지 않았다 — 조기 회수의 최악이
중복 취소가 아니라 **불필요한 조회 1회**다. 강제하려면 `app.refund.pg-timeout` 을 복제해야 하고,
그 복제가 드리프트하면 검증 자체가 거짓이 된다.

### 닫지 않은 것 — 중간 상태

PG 조회가 `READY`/`IN_PROGRESS`/`WAITING_FOR_DEPOSIT`/미상을 돌려주면 **원장을 닫지 않는다.**
`FAILED` 로 닫으면 뒤늦게 성립한 과금이 로컬 `FAILED` 와 영구히 어긋난다. 상한(`unresolved-limit`)
초과 시 Slack + 수동 종결로 넘긴다 — ADR-0018 D2 와 같은 종결 표면이다. 파라미터 테스트로
미상 상태(`SOMETHING_NEW`)까지 넣어 **allow-list 가 아니라 deny-list 였으면 red** 가 되게 했다.

### 웹훅은 진실의 출처가 아니다

`WebhookService` 가 상태를 복구하지 않는다는 것(C-4)을 고치면서, payload 를 믿고 전이시키는
쪽으로 가지 않았다. 서명이 유효한 재전송·순서 역전이 로컬 상태를 되돌릴 수 있다. 대신
`UNRESOLVED` 원장의 `claimed_at` 을 비워 **다음 순회 최우선**으로 만든다. `CLAIMED` 는 건드리지
않는다 — 진행 중인 호출과 경쟁시킬 이유가 없다.

C-9 가 여기서 함정을 하나 막았다: 환불 원장의 `claimForReconcile` 은 `claimed_at < :staleBefore`
만 보므로 NULL 을 못 잡는다. 승인 원장은 두 분기 모두 `claimed_at IS NULL` 을 포함해 작성했고,
통합테스트가 "nudge 직후 lease 만료 전에도 claim 된다"로 관측한다.

### 감사 기록이 거짓이 되지 않게 한 것 둘

- **고아 과금의 승인 원장은 `SUCCEEDED`** 다. 승인은 실제로 성공했고, 되돌려야 한다는 것은
  환불 원장이 소유하는 별개의 축이다. `FAILED` 로 적으면 감사가 거짓이 된다.
- **`ALREADY_CANCELED` 를 재사용하지 않고 `ALREADY_PROCESSED` 를 신설**했다. 둘 다 "이미
  일어났으니 조회하라"로 수렴하지만, 겹쳐 쓰면 원장의 `code` 가 실제로 일어난 사건을
  가리키지 못한다.

### 사용자 응답이 정직해졌다

기존에는 성공 아니면 `PAY-005`(실패) 둘뿐이었다. 결과 불명을 `PAY-005` 로 돌려주면 과금이
성립했을 수 있는 건에 "결제 안 됐다"고 단언하는 것이다. `PAY-012`(409, 확인 중)를 신설했다.
프런트가 처리할 상태가 하나 늘었지만, 늘어난 쪽이 사실이다.

### 검증

- 전 8모듈 guards 그린 · payment-service 모듈 그린
- 승인 원장 통합테스트 8종(실제 MySQL) — fence 의 `INSERT IGNORE` 영향 행 0 · lease 만료 회수와
  generation 증가 · nudge 후 즉시 claim · **T1 만 커밋된 건의 조회 확정(V-4)** · **고아 과금의
  환불 원장 생성(V-8)**
- `ApprovalExecutorTest` — 멱등키 안정성 · PG 정확히 1회 호출 · 중간 상태 4종 파라미터 · verify 가
  `confirm` 을 절대 부르지 않음
- `ApprovalStatusTest` — 전이 전수(정본 맵과 대조, 여기 없는 조합은 전부 거부)
- `PaymentApprovalPropertiesBindingTest` — 등호(`5m == 5m`)도 부팅 실패 · approval 블록 누락 실패

### 미충족

- **실 PG 장애 주입 미검증.** TASKS.md 의 D-020 진입 조건("외부 PG 장애 주입 환경 필요")은 여전히
  미충족이다. crash 경계는 트랜잭션 분리와 `claimForReconcile` 로 **모사**했을 뿐이다.
...
#111(HPA replicas)·#112(user 공개키 seam)가 머지된 덕에 **양 키 도메인 모두 운영 전용 키**로 돌 수
있었다. 세션 1 은 user 도메인에 공개키를 실을 자리가 없어 dev 키쌍으로 우회했고 "운영 전용 키 축
미증명"을 남겼는데, 이번엔 개인키(Secret Manager v3)와 JWKS 게시 공개키의 모듈러스가 일치하는
상태에서 전 구간이 돌았다. **결함을 고치고 다시 측정하니 막혀 있던 축이 열린 사례다.**

### 회전은 되는데 롤백이 안 됐다 — runbook 의 실제 오류

P11(회전 ①~⑤)은 무중단으로 완주했다. 문제는 P12(순서 역전)의 **되감기**에서 나왔다.
`active-kid` 를 구 kid 로 되돌렸는데 401 이 그대로였다 — Secret Manager `versions/latest` 가 여전히
신 개인키를 가리켜, **신 키로 서명하고 구 kid 라벨을 붙인** 토큰이 흐르고 있었다. kid 와 개인키는
쌍이라 둘 다 되돌려야 한다.

그리고 버전을 **비활성화**해서 되돌리려 했더니 더 나빠졌다:

```
FailedPrecondition: Secret Version [.../versions/3] is in DISABLED state.
```

`versions/latest` 는 비활성 버전을 건너뛰지 않는다. 401 이던 것이 Pod `ContainerCreating` 고착으로
악화됐다. **정답은 구 키를 새 버전으로 재업로드하는 것**이었고, 그러자 복구됐다.

`docs/runbooks/user-jwt-key-rotation.md` §4.1 은 초판에 "`active-kid` 되돌림만으로 복구된다" 고
썼다 — **틀린 문서를 내보낸 것**이고, 이 세션이 그것을 실측으로 잡아 같은 PR 에서 고쳤다.

### 게이트가 무엇을 막고 무엇을 안 막는지 드러났다

P12 에서 역전 상태를 만들었을 때 `rollout-convergence-gate.sh` 는 **수렴 판정은 통과**시켰다 —
이미지 digest·replica·구 RS drain 이 전부 정상이기 때문이다. 막은 것은 `--gateway-signing-probe`
단계뿐이었다. 즉 **probe 없이 수렴만 보면 이 상태가 green 으로 지나간다.**

또 역전 실패는 "전 요청 401" 로 시작하지 않는다. 롤아웃 중에는 구/신 Pod 이 섞여 `401 200 200`
같은 **부분 장애**로 나타나고, 수렴 후에야 전면화한다. 계획서의 기대 서술이 이 점을 놓치고 있었다.

### P14 를 축소 없이 한 방법 — 분리를 없앴다

세션 1 은 CPU 쿼터(`CPUS-ALL-REGIONS` = 12, 증설 **두 번 자동 거부** · 콘솔에서도 조정 불가)
때문에 노드 2대 + loadgen VM 2 vCPU = 10 으로 갈 수밖에 없었고 HPA 피크를 못 받았다.

이번엔 **loadgen VM 을 없애고 k6 를 클러스터 안 Job 으로** 돌렸다. VM 2 vCPU 를 노드로 돌려 3대를
확보하니 앱 여유가 4994m → 7964m 가 됐다. 노드를 늘리면 DaemonSet 고정비도 늘지만 할당가능 CPU
증가폭이 훨씬 커서 순증이다. ADR-0004 가 별도 VM 을 고른 근거는 "집 인터넷 경유 기각"이라 VPC 내
co-location 과는 다른 축이지만, **부하 발생기가 앱과 CPU 를 경쟁**하는 것은 사실이라 편차로 남겼다.

그리고 첫 측정은 무효였다 — 298 req/s 로 밀었더니 60% 실패였는데 **전부 429**였다. fail-closed
RateLimiter 가 설계대로 동작한 것이고 한도는 키당 40 req/s 다. 한도 아래로 재측정해야 지연 수치가
의미를 갖는다. **계획서 P14 에 이 전제가 없었다.**

결과: 인증 경로 p95 **23.61ms** vs 공개 경로 **13.67ms** — 인증 오버헤드 **+9.9ms**. 이 값에는 서명
외에 JWT 파싱·JWKS 조회·Redis deny 조회·헤더 재작성·다운스트림 홉이 전부 포함되므로, PR3d-a 의
마이크로벤치(RSA2048 서명 p95 1.80ms)와 직접 비교할 수 없다. auth 실패 기저율은 **0** 이라,
이 카운터는 비율 임계가 아니라 **증가 자체를 사건으로 보는** alert 이 맞다.

### 미충족

- **오버셀링 정합성 미검증** — 계획서가 지정한 `order-concurrency.js` 는 1000 VU·DB seed 의존의
  Phase 3 시나리오라 P14 의 목적(지연 분리·auth 기저율)과 축이 달라 목적 전용 스크립트를 썼다.
- **event-loop lag 직접 계측 없음**(관리 포트 probe 대리) · **auth alert 임계 미확정**(입력값만 산출).
- **측정에 co-location 경합이 섞여 있다** — 별도 VM 측정과 직접 비교 불가.
- **User 도메인 회전 미실증** — P11/P12 는 내부 토큰 도메인이다. JWKS 를 Service 로 조회하는
  User 고유 축(runbook §1.1)은 여전히 미검증.
...
gateway 가 JWKS 를 **발급자의 Service** 로 조회하기 때문에 회전 중 구 Pod 에 떨어질 수 있다는
§1.1 — 과 그것을 막는 §3.1 전수 확인 gate 의 유효성이 미검증이었다.

환경은 minikube 를 골랐다. 이 축은 k8s Service 의미론 그 자체라 클라우드 종속이 없고,
Secret Manager/WI 축은 세션 2 가 이미 실증했기 때문이다. 다만 그래서 **이 드릴의 결론은
"운영 완주" 가 아니라 "축의 실증"** 이다.

### 1차 시도는 실패했고, 그것이 방법을 바꿨다

금지된 순서(①+③ 동시)를 적용하고 롤아웃 중 1초 간격으로 때렸는데 401 이 안 나왔다. gateway 로그를
보니 첫 신 kid 토큰이 발급된 바로 그 순간 이미 전 Pod 가 수렴해 있었다. **1회 표본으로 판정할 수
없는 레이스**라는 뜻이라, 레이스를 제거한 결정적 재현으로 바꿨다.

### 발견 — runbook §1.1 의 기술이 부정확했다

skew 상태를 고정하고 gateway 를 5회 재시작하며 121회 로그인했더니 **전부 같은 Pod** 에서 나왔다.
무작위 분배라면 `0.75^121 ≈ 10^-15` 다. 같은 Service 를 **요청마다 새 커넥션**으로 20회 부르니
7회가 다른 Pod 로 갔다. 즉 gateway 는 커넥션을 풀링해 **한 Pod 에 고정**된다.

이게 runbook 이 쓴 "어느 Pod 가 응답할지 보장되지 않는다" 보다 **나쁘다**:

- 한 번 구 Pod 에 물리면 지속된다. `jwks-refresh-cooldown: PT10S` 를 기다려 재시도해도 같은
  커넥션이라 같은 Pod 로 간다 — **자가치유가 없다.**
- 반대로 신 Pod 에 물린 gateway 인스턴스는 끝까지 정상이다 → **일부 사용자만 401**. "전 요청 401"
  을 장애 신호로 기다리면 놓친다. (세션 2 가 내부 토큰 P12 에서 "부분 장애로 시작한다" 고 적은 것과
  같은 결이지만, 원인은 롤아웃 중 Pod 혼재가 아니라 **커넥션 고정**이다.)
- 로그인 경로와 JWKS 경로는 별개 풀이라 서로 다른 Pod 에 물릴 수 있다. 그 조합이 §1.1 장애다.

### G1/G2

경로를 각각 Pod IP 로 고정해 단일 변수만 바꿨다 — **토큰 kid 는 양쪽 동일**하고 JWKS 출처만 다르다:

| | 로그인 | JWKS | 결과 |
|---|---|---|---|
| D1 | 신 Pod | **구** Pod | `kid=peekcart-drill-2026` **401 × 10/10** |
| D2 | 신 Pod | 신 Pod | `kid=peekcart-drill-2026` **200 × 10/10** |

같은 skew 상태에서 §3.1 을 돌리니 구 kid 만 가진 Pod 3개를 그대로 드러내며 차단했다(G2).
**③ 진입 전에 돌렸다면 이 회전은 막혔다.**

정상 순서 ①~⑤ 는 완주했고, ③ 롤아웃 전 구간 연속 요청 79회에서 **401 은 0건**이었다(kid 전환
경계 포함). 순서를 지키면 창이 안 열린다.

### 남긴 것

- **Secret Manager/CSI/WI 축 미재현** — 드릴이 k8s Secret 으로 대체했다. User 도메인을 **GKE 운영
  기구로** 완주한 기록은 여전히 없다(runbook §6 에 그 항목만 남겨두었다).
- **자연 롤아웃에서의 발생 빈도 미측정** — 6회 시도 전부 음성. "열리면 이렇게 된다"만 증명됐고
  "언제 열리는가"는 아니다.
- 드릴 overlay 는 `workload-key-ownership-lint` 범위 밖이고 개인키 SPC 배타 소유 규칙을 의도적으로
  위반한다 — **운영 매니페스트 승격 금지**.
...
D-002a([#117])가 "2차 병목은 CPU 도 Hikari 풀도 아니다 — **MySQL 내부**가 남았다" 로 넘긴
그 "MySQL 내부" 를 열었다. **세 겹이었고 셋 다 다른 축**이다.

| | 천장 | 정체 | 증거 |
|---|---|---|---|
| ① | 동기 주문 생성 | **MySQL 자기 CPU 한도(500m)** | 500m→2000m 로 54→158.6 건/초 **×2.94**, p95 2.38s→879ms |
| ② | 비동기 예약 사가 | **outbox 폴링 설정**(100건/5s=20/s) | CPU 4배에 +14%뿐 → 완화 시 16.3→77.3 **×4.8** |
| ③ | 발행의 가용성 | **스케줄러 스레드 1개** | 형제 잡 185초 동안 발행 **완전 정지**, 해제 13초 후 재개 |

①이 D-002a 가 놓친 축이다. 그 증적이 배제한 "CPU" 는 **product-service 의 CPU** 였고,
`gke-d002a/kustomization.yml` 의 `patches` 가 1건뿐이라 **MySQL 은 그 세션 내내 base 값 500m**
로 돌았다. 그 축은 한 번도 변경되지 않았다(이번 측정 중 498m/500m 한도 도달 실측).

②의 병목 위치는 추정이 아니다 — Kafka consumer **LAG 0**, 그리고 예약 건수가 outbox
`PUBLISHED` 와 **정확히 일치**(3,300=3,300). 소비자는 굶고 있었다.

### D-002c — 락이 하는 일이 없다

경합은 **replicas=1 에서 구조적으로 불가능**했다. 예약 소비 스레드가 시스템 전체에 1개
(`replicas=1` × listener `concurrency` 기본 1)라 분산 락에 경쟁자가 없다 — 부하와 무관하다.

replicas=3 으로 올리니 나타났고, 계획서 P9 의 판정이 그대로 성립했다:

```
PRD-004(락 대기 3s 초과)  0      ← 락은 항상 즉시 획득된다
OptimisticLock 충돌       7      ← 그런데도 충돌한다
DLQ                       7      ← 충돌 수와 정확히 일치
P10 불변식: 음수재고 0 · 품목별 diff 0   ← 오버셀링 없음
```

락이 커밋을 직렬화한다면 낙관락 충돌은 **불가능**해야 한다. 락이 멀쩡히 획득되는데 충돌한다는
것은 **락 해제가 커밋보다 먼저 일어난다**는 코드 사실(V5)이 런타임에 드러난 것이다.
정합성은 `Inventory @Version` 이 홀로 지켰고, 대가는 DLQ 7건이다.

즉 분산 락은 지금 아무 일도 하지 않는다 — replicas=1 에서는 경쟁자가 없어서, replicas=3
에서는 커밋을 감싸지 못해서. **락은 필요해지는 바로 그 시점에 틀린 상태가 된다.**

### 정합성

첫 clean run: 주문 9,599 → 예약 9,599(RESERVED 9,392 + RELEASED 207), **DLQ 0**.
배수 552초 — 2분 부하가 남긴 사가를 비우는 데 9.2분(폴링 기본값 기준).

### 미충족

- **spread 대조군 오염** — 직전 run 배수 중 초기화해 두 run 이 섞였다(`RESERVED 8,250` >
  해당 run 주문 6,634). contend vs spread 비교는 무효이고, 결론은 **replicas 1↔3** 대조에 근거한다.
- **목표 300 rps 미달성** — VU 200개의 구조적 상한(~80 iter/s). "300rps 를 넣은 결과" 가 아니라
  **"200 동시 사용자로 포화시킨 결과"** 다.
- **P6(MySQL 되돌리기) 미수행** — ①은 한도 도달을 직접 관측(498m/500m ↔ 1,259m/2000m)했으나
  되돌려 천장이 복귀하는지는 확인하지 못했다.
- **외부 연동 미검증** — Toss 자격증명은 smoke placeholder.
- **Codex 리뷰 미호출**(사용자 지시) — **"P0/P1 = 0" 주장 없음.**
...
- **D-002a 증적의 "잔여 과금 자원 0 확인" 은 사실과 달랐다** — PVC 3개가 남아 있었다.
  회수 완료 + `cleanup.sh` 를 실제 회수하도록 수정(잔여를 exit code 로 검증) + ADR-0004 본문 정정.
  이번 세션 종료 시 그 수정본이 orphan PD 3개를 **실제로 삭제**하며 검증됐다.
...
계획서: `docs/plans/done/task-d028-topic-describe-setup-race.md`

## D-027 ② — Codex 리뷰 응답 실패 판별 ([PR #128](https://github.com/Kimgyuilli/PeakCart/pull/128), 2026-09-20)

착수 명목은 "Codex 리뷰 JSON 을 severity 표로 렌더링" 이었다. 실물 282개를 전수 조사하자
무게중심이 옮겨갔다.

`/plan` 과 `/work` 의 결과 처리 절은 지적이 0건이면 자동 통과로 규정한다. 그런데 모델이
결론을 내지 못한 런의 결과도 0건이다. 282개 중 **44개(16%)가 빈 응답**이었고, 원인은
사용량 한도 9 · MCP transport 실패 9 · OAuth 갱신 실패 5 · 모델 capacity 1 등으로 갈렸다.
"리뷰가 깨끗했다" 와 "리뷰가 안 돌았다" 가 구분되지 않는 false-green 이었다.

`scripts/codex-review-render.sh` 의 종료 코드를 셋으로 나눠 막았다 — 0 정상(지적 0건 포함) ·
1 사용법 오류 · 2 리뷰 실패. 커맨드 문서에 "2를 0으로 접지 말고 리뷰 상태를 `미결` 로
기록하라" 는 규칙을 함께 박았다.

### 구현 중 드러난 것 — Codex 리뷰가 전면 중단 상태였다

이 PR 의 diff 리뷰를 시도하다 렌더러가 400 을 잡아냈다. 현재 API 는 structured output
스키마의 `required` 가 properties 의 모든 키를 포함할 것을 요구하는데, 두 스키마가
`scope` 와 `suggested_disposition` 을 빠뜨려 **모든 `codex exec` 호출이
`invalid_json_schema` 로 거부**되고 있었다.

계획 단계에서는 이 스키마 수정을 "Codex 출력 계약 변경이라 등급이 오르고, 렌더러 없이는
변경이 먹혔는지 확인할 방법도 없다" 며 이월했다. 그 판단은 **리뷰가 정상 작동 중**이라는
전제 위에 있었고 그 전제가 틀렸다. 이월 조건으로 걸어둔 "렌더러 선행" 이 렌더러가 400 을
잡아낸 그 자리에서 충족됐다. 사용자 확인 후 범위에 편입하고 등급을 M 에서 L 로 올렸다.

부수 효과로 과거 이력 1267건에서 전부 결측이던 두 필드가 채워져 온다. 프롬프트가 말로
요구하던 항목 분류를 스키마가 강제하게 됐다.

### 검증 도구가 검증 대상과 같은 출처면 둘 다 틀려도 통과한다

diff 리뷰 라운드 1이 P1 2건을 잡았다. 그중 하나가 뼈아프다 — `items` 가 배열인지 보지 않아
`{"items":{}}` 가 exit 0 과 `지적 0건` 으로 샜다. **이 작업이 막으려던 false-green 이 구현
안에 그대로 있었다.** 게다가 실제 이력 회귀(V9)의 기대값을 렌더러와 같은 jq 식으로 계산한
탓에 회귀 검사도 함께 눈을 감았다. 기대값 계산을 다른 표현으로 분리했다.

V9 는 애초에 238/44 를 하드코딩했다가 리뷰를 한 번 더 돌리자마자 캐시가 늘어 깨졌다.
스냅샷은 계약이 아니다. 종료 코드 집합과 실패 조건의 일치를 보는 불변식으로 바꿨다.

### 검증

`./gradlew test` BUILD SUCCESSFUL(제품 코드 무변경) · selftest V1~V15 35건 통과 ·
bats 62건 통과 · `plans-index --check` 최신 · `writing-lint` 커밋 6건과 본문 통과.

### 미충족 / 이월

미충족 없음. 계획서 P1~P6 전부 구현, 검증 15행 전부 통과.

이월 2건은 PR 본문 §이월 에 있다. ① 표 검증을 문자열 매칭에서 헤더 파싱으로(재검토 조건:
컬럼 구성이 한 번 더 바뀔 때) ② `.cache/codex-reviews/` 실패 파일 보존(V9 회귀 입력).
둘 다 국소적이라 부채 행으로 승격하지 않았다.
...
- **D-026 N8** — "배속 미측정인데 해소로 적지 않는다" 를 닫았다. **완료로 전환.**
- **D-002a 읽기 경로 잔여** — 캐시 OFF 공통 천장(detail 378.1 ≈ list 380.0)의 정체가
  **MySQL 자신의 CPU 한도**임을 확정했다. 500m→2000m 에서 detail OFF **393→904 rps**,
  MySQL 스로틀 **147.6초→0.1초**(60초 런). 되돌림 A′ 가 **392.6(+0.2%)** 로 복귀해
  재시작·드리프트 효과가 아님을 확인했다. #119 가 **주문 경로**에서 낸 결론이 읽기 경로에서도
  성립한다.

**명명 정정**: 이 작업을 착수 시 "D-002 P5" 로 불렀으나 **틀렸다.** `task-d002-bc-session.md:85`
의 P5(MySQL `limits.cpu` 흔들기)는 주문 경로에서 이미 수행됐고 `TASKS.md` 도 D-002 를 완료로
적는다. 완료된 행을 되살리지 않도록 **D-002a 읽기 경로 잔여**로 개명했다(계획 리뷰 지적 #9).

### 헤드라인 숫자를 바꿨다 — 세션 간 비교가 오염돼 있었다

`×1.23 → ×2.30` 이 눈에 띄는 수치지만 **쓰지 않았다.** 캐시 OFF 는 기준선과 ±4% 로 겹치는데
캐시 ON 만 **+16.7%** 올라 대조군 게이트가 실패했고, 원인은 두 경로의 병목이 다르다는 데 있다
— OFF 는 MySQL 500m 에 묶인 **DB 바운드**(환경 이동에 둔감), ON 은 **앱/CPU 바운드**(새 노드·
새 GKE·새 k6 의 이득을 받음). **바닥은 고정, 천장만 이동**이다.

그래서 계획서 §3-3 이 미리 정한 **같은 세션 paired ratio** 로 판정했다:
...
**D-026 이 한 일은 detail 을 빠르게 만든 것이 아니라 캐시 적중 천장에 도달시킨 것**이다.
그리고 **블록 B 가 블록 A 의 해석을 정정했다** — 1.01 은 동률이 아니라 **둘 다 ≈900 rps 천장에
눌린 착시**였고, 천장을 풀면 detail 은 list 의 **약 81%** 다(Redis 를 `product`+`productStock`
**두 번** 탄다). 되돌림 블록이 없었으면 "동률 달성"으로 잘못 적었을 것이다.

### 게이트 5개 중 2개 실패 — 통과로 적지 않았다

| 게이트 | 편차 | 판정 |
|---|---|---|
| 재현 detail OFF / list OFF | +4.0% / +3.3% | 통과 |
| 대조군 list ON | +16.7% | **실패** → §해석 (위) |
| 되돌림 detail OFF | **+0.2%** | 통과 → CPU 효과 ×2.30 **확정** |
| 되돌림 list OFF | +11.5% | **실패** → list 배속은 **방향만**, 숫자 미확정 |

list OFF 블록 B 의 산포 44.5% 도 같은 자리를 가리킨다 — 개별값 **782.7 / 1406.4 / 1408.4** 로
첫 런만 낮다. `Recreate` 로 식은 버퍼풀이 덜 데워진 것이고, 블록 워밍업(전 상품 풀스캔 3회)이
list 경로에는 부족했다.

### 하네스 결함 4건 (전부 이 세션에서 발생·수정)

1. **실행 중인 스크립트를 편집**해 런 1건이 중단됐다. bash 는 스크립트를 오프셋으로 읽으므로
   실행 중 편집은 재개 위치를 어긋나게 한다. 재실행으로 복구.
2. **`rollout status` 직후 `items[0]`** 이 Terminating 중인 구 Pod 를 집어, `list:on` 3런이
   전부 "런 중 재시작"으로 오판·폐기됐다 — **대조군(N4)이 통째로 날아갈 뻔했다.** Pod 1개
   수렴 대기 + `--field-selector=status.phase=Running` 으로 수정.
3. **CPU 단위 비교** — 쿠버네티스가 `2000m`→`2`, `3000m`→`3` 으로 정규화하는데 문자열로
   비교해 조건이 맞는데도 블록 B 가 중단됐다. 밀리코어 환산으로 수정.
4. **캐시가 셋인데 둘만 분해**했다(`product`·`productStock`·`products` 중 `product` 누락).
   핵심인 `productStock` 은 잡혔으나 상세의 상품정보 캐시 거동은 증적에 없다.

폐기 검사(런 전후 Pod UID 대조)는 **설계대로 동작했다** — 오염된 런이 조용히 섞이는 대신
드러나서 ②를 찾을 수 있었다.

### 선재 결함 (손대지 않음)

`k8s/overlays/gke-d002a` 는 `../../base/*.yml` 을 파일로 직접 참조해 kustomize root 이탈 제한에
걸린다 → **기본 설정으로 한 번도 렌더된 적이 없다**(HEAD 상태에서도 동일 실패 확인).
구조 수정은 범위 밖이라 `--load-restrictor LoadRestrictionsNone` 을 정본 렌더 명령으로 명시하고
overlay 주석에 사유를 남겼다.

### 이번 세션이 남긴 자산

- `k8s/overlays/gke-d002a-mysql-500m` / `-2000m` — **두 조건이 각각 렌더되는 이름**을 갖는다(N5).
  D-002a 가 조건을 손으로 patch 하다 MySQL 을 내내 500m 로 돌린 사고의 재발 방지.
- `loadtest/scripts/d026-run-cell.sh` / `-block.sh` / `d026-collect.py` — 런 프로토콜·블록 전환·
  집계. counter **차분**, Pod 동일성 검사, 게이트 판정을 스크립트가 수행한다.
- `loadtest/cleanup.sh` 에 **Internal LB 잔여 검사** 추가(forwarding-rules · backend-services ·
  health-checks) — `measurement-lb.yml` 이 만드는 자원이고, D-002a 의 PVC 3개 누수와 같은 계열.

### 미해결

- **list 경로 CPU 배속 미확정**(되돌림 실패). 재측정 시 블록 워밍업에 list 경로 포함 필요.
- **캐시 ON 경로가 MySQL CPU 에 반응한 이유 미분리** — list ON 은 캐시 miss 0인데 894.9→2249.2.
  후보: 노드 레벨 CPU 경합 · cgroup `cpu.stat` 판독 범위. **추측으로 메우지 않았다.**
- **2000m 에서의 새 천장**(detail OFF 904) 정체 미분리.
- **base `mysql.yml` CPU 상한 승격은 ADR 대상** — 전 overlay 에 영향. #119(주문 경로)와 이번
  읽기 경로 결과를 **묶어서** 쓰는 것이 맞다. **`D-029` 로 부채 등록**(2026-09-22).
  그 ADR 은 아래 두 미해결을 **모르는 채로 결정한다는 사실**을 적어야 한다 — list 배속 미확정과
  2000m 의 새 천장.

## 하네스 — 규약 강제를 문서에서 검사기로 ([PR #129](https://github.com/Kimgyuilli/PeekCart/pull/129), 2026-09-22)

> D-026 세션 종료 후 같은 PR 에서 이어진 작업. 측정과 무관한 별건이다.
> 계기는 PR #129 본문이 `docs/conventions/writing.md` 를 전면으로 어긴 채 나간 것이다.

### 무엇이 깨졌나

| 규칙 | 원문 | 실제 |
|---|---|---|
| PR 본문 어미 `~습니다` | writing.md §3 첫 줄 | `~다.` **46문장** / `~습니다.` **0** |
| 볼드 섹션당 최대 2 | §3 | Why 3 · What 6 · Test plan 3 |
| 수사·반전 금지 | §3 | `### 헤드라인 숫자를 바꿨다` 등 3개 |
| 시행착오는 audit 에 | §3 마지막 줄 | §미충족에 하네스 결함 4건 나열 |
| 섹션 목록 | `/ship` §4 | 규격에 없는 `범위 밖`·`부수 변경` 추가 |

### 원인 — 초록불이 계약을 덮지 않았다

**검사기를 돌리고 규약을 읽지 않았다.** `/ship` §4-1 이 "정본은 writing.md" 라 적고 lint
명령을 주는데, lint 만 돌리고 통과를 규약 준수로 읽었다. writing.md 는 그 시점에 열지 않았다.

그리고 **lint 가 그 규칙을 검사하지 않았다.** 어미 검사가 없었고, 기존 `서술 종결` 검사는
`mode == 'title_body'` 블록 안에 있어 **커밋 제목 전용**이었다. `--file` 모드에는 돌지 않는다.
볼드는 규약이 "최대 2개" 로 규정하는데 lint 가 경고로 분류해 3건을 남긴 채 통과시켰다.

D-027 ②가 `exit 0` 과 "리뷰가 안 돌았다" 를 가른 것과 같은 계열이고, 이번엔 우리가 그쪽에 섰다.

### 처방 1 — 검사 가능한 것은 검사기로

- **어미**(`~습니다`) PR 본문 전용 **오류** 신설
- **볼드 과다** PR 본문에서 경고 → **오류** 승격 (커밋은 경고 유지)
- lint 통과 시 **덮지 않는 항목 3줄을 출력** — writing.md §6 은 자동으로 읽히지 않아
  0원이자 0효과다. `/ship` 이 이미 lint 를 돌리므로 그 출력에 얹으면 커맨드 파일을 늘리지
  않고 판단 지점에 닿는다
- writing.md §6 에 **덮는 것 / 덮지 않는 것** 표를 갈라 적었다. 수사와 자기 서사는 기계로
  못 잡으므로 **무엇이 안 덮이는지를 문서가 말해야** 한다

**경어체 판정은 목록이 아니라 종성으로 한다.** 초안은 `습니다|입니다|합니다|됩니다|십니다`
5개 목록이었고 `탑니다`·`아닙니다`·`깨집니다`·`걸립니다` 를 전부 오탐으로 잡았다. 경어체는
어간에 `-ㅂ니다` 가 붙는 형태라 목록으로 못 덮는다. `니다.` 로 끝나고 **앞 음절 종성이 ㅂ**
인지 보면 평서형 `아니다.` 는 걸리고 활용형은 통과한다. self-test 픽스처 18건(위반 적발과
오탐 방지 양쪽).

### 처방 2 — 커밋 분류는 git 훅이 막는다

같은 세션에서 `hpx_staged_category_check` 가 `mixed` 를 반환했는데 **그대로 커밋했다**
(`cf54016`). 그 함수는 1 을 정확히 반환한다. 커버리지 공백이 아니라 **강제 공백**이었고,
문서(`/ship` §3)가 유일한 강제 수단이면 건너뛰어진다.

초안은 Claude Code PostToolUse 훅이었는데 **막지 못하고 알려주기만 했다.** 정보를 무시해서
생긴 문제를 정보로 막으려는 꼴이라 구조를 바꿨다.

| | 위치 | 역할 |
|---|---|---|
| `.githooks/pre-commit` | 스테이징 확정 후, 커밋 객체 전 | **mixed 커밋 차단** |
| `.claude` PreToolUse 훅 | 명령 문자열과 git config | **우회 경로 차단** |

- **PreToolUse 로 분류 검사를 못 하는 이유**: `git add && git commit` 복합 명령에서 add 전에
  돌아 직전 스테이징을 본다. git 훅에는 그 문제가 없다
- Claude 훅은 역할을 좁혔다 — `--no-verify`(`-n`, 결합형 `-nm` 포함)와 `core.hooksPath`
  미설정을 막는다. 둘 다 git 상태를 안 보므로 PreToolUse 에서 **실제 차단**이 된다
- **fail-open 을 fail-closed 로** 바꿨다. 초안은 `shared-logic.sh` 가 없으면 조용히 통과시켰는데,
  그건 검사가 있는 척하며 아무것도 안 하는 상태다
- 탈출구 `HPX_ALLOW_MIXED=1` 을 양쪽이 함께 존중한다. 막다른 길이 생기면 훅을 지우게 되고
  그러면 강제가 통째로 사라진다
- `.githooks/*` 분류 규칙이 없어 `src` 로 떨어지고 있었다 → `chore` 추가

검증: pre-commit 3/3(혼합 차단·단일 통과·탈출구 통과, 격리 저장소에서 커밋 수 0 확인),
PreToolUse 가드 6/6. 가드가 살아 있다는 부수 증거로, 테스트를 짜다 명령 문자열이 패턴에
걸려 두 번 차단됐다.

### 어디에 강제를 두는가 — 토큰 비용

커맨드 파일은 **매 호출 컨텍스트에 실린다**(`plan.md` 10,025자 · `ship.md` 8,641자 ·
`work.md` 7,757자, 합 26,423자). 스크립트와 규약 문서는 실리지 않는다.

| 수단 | 규칙이 존재하는 것만으로 드는 비용 | 이번 실증 |
|---|---|---|
| 훅 | **0** (컨텍스트 밖 실행, 통과하면 침묵) | 실제로 막는다 |
| 스크립트 검사 | 0 (본문 미적재, 출력만) | 46건 적발 |
| 커맨드 파일 산문 | **매 호출 전량** | `/ship` §3 이 이미 적혀 있었으나 **못 막았다** |

가장 싼 수단이 가장 잘 막는다. 컨텍스트 밖에서 도는 것이 싸고, 싼 것이 사람의 성실성에
덜 기댄다. 이번에 추가한 둘은 커맨드 파일을 **한 글자도 늘리지 않았다**.

**부채로 남는 것**: 같은 세션에서 `/plan`·`/work` 에 Codex 승인 게이트 **+26줄**을 더했다.
규칙 자체는 사용자에게 묻는 동작이라 커맨드에 있어야 하나, 함께 넣은 근거 문단은 D-027 ①이
지목한 유형이다. **지금 걷어내지 않는다** — D-027 ①의 재개 조건이 "규칙이 안정된 뒤" 이고
사유가 "방금 만든 규칙의 설명을 바로 걷어내면 오적용 시 원인을 모르게 된다" 이다.
D-027 ① 재개 시 1순위 대상.

### 미해결

- `cf54016` mixed 커밋은 **그대로 뒀다**(force push 필요, 사용자 판단으로 보류). 앞으로는 훅이 잡는다
- `core.hooksPath` 는 클론마다 설정해야 한다. Claude 경로에서는 PreToolUse 가드가 첫 커밋을
  막고 안내하지만, **IDE 나 터미널에서 미설정 클론에 직접 커밋하면 빠져나간다**.
  git 이 `.git/hooks` 를 버전 관리하지 않는 한계라 완전히 닫을 수 없다
...
`k8s/base/infra/mysql/mysql.yml` `limits.cpu` **500m → 2000m**, `requests.cpu` **250m 유지**.

500m 은 읽기와 쓰기 양쪽의 **공통 천장**이었다 — 주문 경로 498m/500m 한도 도달(×2.94 손실),
읽기 경로 60초당 147.6초 스로틀. D-002a 가 "MySQL 내부가 남았다"로 닫은 그 "내부" 안에
MySQL 자신의 CPU 상한이 미분리로 들어가 있었다.

`requests` 를 같이 올리지 않은 이유: A→B→A′ 되돌림 실험이 흔든 손잡이는 **`limits` 하나**였고
×2.30 은 그 손잡이의 효과다. 동반 상향은 측정 범위를 넘는다. 대가는 2000m 조건 실사용
1,259m 중 **250m 초과분이 보장되지 않는 버스트**라는 것이고, 이것을 ADR 부정적 영향에 적었다.

### 계획의 전제 두 개가 구현 중 틀린 것으로 드러났다

둘 다 **기준선에서는 구별이 불가능했고, base 를 올리는 행위가 갈라줬다.**

1. **상속 overlay 는 7개가 아니라 5개다.** 계획서 §2 렌더 표가 `gke-d002bc` 를 base 상속으로
   분류했으나 그것은 `patches/mysql-deployment.yml` 로 500m 을 **자기가 박고 있었다.**
   patch 값과 base 값이 **둘 다 500m 이라 렌더 결과가 같았고**, 초안은 kustomization 의
   `resources:` 목록만 보고 상속이라 단정했다. base 승격 후 500m 에 남은 것이 patch 를 가진
   쪽이다. → **같은 값일 때 상속과 명시는 렌더 결과로 구별할 수 없다.** 표에 출처 열을 추가했다.
2. **거짓이 될 주석은 2곳이 아니라 3곳이었다.** 놓친 것은
   `gke-d002bc/patches/mysql-deployment.yml:31` 의 `# P6 에서 흔드는 손잡이. base 와 같은 값으로
   시작한다.` 로, `resources:` 블록 **안쪽 인라인 주석**이어서 파일 헤더를 훑는 스윕에
   걸리지 않았다. **문구 grep** 으로 돌렸을 때 잡혔다.

### 트레이드오프 판정 — TASKS.md 가 적은 1순위가 1순위가 아니었다

D-029 행은 "minikube 가 2 vCPU 를 MySQL 에 내줄 수 있는가"를 먼저 적었으나 **거의 해소된다**:
`limits` 는 스케줄링에 쓰이지 않고(`requests` 만 쓴다), minikube(4코어) base requests 합은
**2100m < 4000m** 로 변경 전후가 같으며, 현재 `limits` 합은 이미 **7250m = 4코어의 1.8배**로
오버서브스크립션 상태다. 이 변경이 새 성질을 도입하는 게 아니다.

실제 쟁점은 **`requests` 처분**이었다. 그리고 그것을 기각한 근거가 증적 §미해결 2
(캐시 miss 0 인 경로가 DB CPU 에 반응 — 후보는 노드 레벨 경합)와 **같은 미분리 항목**이다.
즉 이 결정은 "노드 경합을 모른다"는 이유로 `requests` 를 건드리지 않았다.

### 검증 (렌더 수준)

| | 결과 |
|---|---|
| overlay 8개 렌더 | **8/8** · 상속 5개가 2000m 로 함께 이동 · patch 3개 불변 · `requests` 8/8 불변 |
| 주입 A — base `limits` 제거 | **통과.** 상속 5개가 `lim=None`, 자체 patch 2개는 500m 유지(대조군 성립) |
| 주입 B — `-500m` patch 제거 | **통과.** `lim=2000m` 으로 상승 → **이 overlay 의 역할이 대조군으로 반전**됐음이 실증 |
| 거짓 주석 | 3곳 정정 후 0건 |
| ADR 인덱스 | 파일 27 = 행 27 · 번호 순서 일치 |

### 측정 overlay 처분 — 삭제하지 않았다

`gke-d002a-mysql-2000m` 은 base 와 값이 같아져 **no-op** 이 됐다. 그래도 남긴다:
읽기 경로 `list` 배속이 되돌림 게이트 실패(+11.5%)로 **미확정**이라 재측정이 남아 있고,
그때 필요한 것은 A/B 대조 쌍이다. 값을 지우고 base 상속에 맡기면 "2000m 으로 쟀다"는
사실이 파일에서 사라진다. 이제 실질 손잡이는 짝인 `-500m` 쪽이다.

### 이월 (ADR 에 "모르는 채로 결정한다"로 명기)

1. 읽기 경로 `list` 배속 미확정 — 되돌림 +11.5%, 블록 B 산포 44.5%(첫 런만 낮음 = 버퍼풀 미가열).
   재측정 시 블록 워밍업을 list 경로까지 확장
2. 2000m 에서도 남는 천장 **904 rps** — 그 셀 스로틀 0.1초이므로 **MySQL CPU 가 아니다.** 정체 미분리
...
## Phase 4 종결 — 구현 ①~⑥ 완료 · L-004 는 D-030 으로 이월 (2026-09-22)
