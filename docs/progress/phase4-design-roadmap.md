...
| A2 | 멀티모듈 구조 | `common` 모듈 경계(이벤트 DTO·공통 예외·응답 포맷 한정), 서비스 모듈 의존 규칙, 빌드/이미지 산출 계약 | L-016a(digest 고정), D-016(image promotion 자동화) |
...
2. **A2 (멀티모듈 구조 ADR)** 착수 — `common` 경계·의존 규칙. ADR-0010 §D1 의 5개 서비스 = 5개 모듈. 이후 A3~A4, 구현 ①~⑥ PR 단위.
