...
> 입력: `docs/learning/18-phase4-prep-design-debt.md`(L-001~L-022 도출 기록) + `docs/TASKS.md §개발 부채`(D-002 추적).
> 이 문서는 **실행 로드맵**(승격·시퀀싱·게이트)이다. 학습/회고는 `docs/learning/18-phase4-prep-design-debt.md` 와 별개.
> 항목이 모두 승격/이관/폐기되면 본 문서와 학습 기록의 참조 상태를 함께 정리한다.
...
### 작업 3 — Tier C: 발행 경로 resilience 하드닝 → **D-013 대기**
...
| **D-013** | 발행 경로 resilience 하드닝(L-010/012) | 1 | 대기 |
...
1. 다음 구현 착수 시 `TASKS.md` 의 D-013 상태를 `🔄 진행 중` 으로 갱신한다.
2. D-013 완료 후 여유가 있으면 D-014 선택 승격 여부를 재검토한다.
