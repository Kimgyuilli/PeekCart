...
> **✅ 버킷 3 종료 (2026-09-18)** — 두 항목 모두 게이트대로 **실측 후** 처분됐다.
> - **L-013**: [#84](https://github.com/Kimgyuilli/PeakCart/pull/84) 해소. 결정적 재현으로 양방향 lost update 확증 → `@Version` 승격 + 충돌 정책 확정.
> - **L-007**: [#120](https://github.com/Kimgyuilli/PeakCart/pull/120) 측정 세션에서 **전제 소멸 + 표면 이동** 확인 → **D-025 로 흡수**.
>   게이트는 충족됐다(replicas=3 동일-상품 경합: `PRD-004` 0 ↔ 낙관락 충돌 7 → DLQ 7). 다만 **측정 대상이 코드에서 옮겨가 있었다** —
>   주문 생성은 더 이상 재고를 차감하지 않고(ADR-0012 D3 사가), 락은 consumer 로 이동했으며 그곳에서 불변식이 **반대로**
>   성립한다(락 ⊂ 트랜잭션). "자연 해소" 가 아니라 **이동 후 역전**이다.
>   **후속 종결(2026-09-18, [#123](https://github.com/Kimgyuilli/PeakCart/pull/123))**: 흡수처인 D-025 가 ADR-0025 로 닫혔다 — 분산 락 제거, `@Version` 단일 수단,
>   jitter 재시도. L-007 이 남긴 "재고 차감 retry 정책 미정"도 여기서 확정됐다(DLQ 전량 소진의 원인은
>   재시도 부재가 아니라 **jitter 부재 lockstep** 이었다).
>
> 이로써 이 로드맵의 **미결 버킷은 없다**.
...
| **D-025** ← L-007 | L-007(전제 소멸 후 표면 이동) | 3 | ✅ 처분 — D-025 로 흡수 ([#120](https://github.com/Kimgyuilli/PeakCart/pull/120)) |
| (해소) | L-013 | 3 | ✅ 해소 ([#84](https://github.com/Kimgyuilli/PeakCart/pull/84)) |
...
3. ~~버킷 3 게이트(17편 후속 부하 세션)는 별도 task 로 추적.~~ ✅ **종료 (2026-09-18)** — L-013 해소([#84]), L-007 은 D-025 로 흡수([#120]). §4 참조.
