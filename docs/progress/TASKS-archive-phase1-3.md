# PeekCart — Task 관리 (Phase 1~3 아카이브)

> ⚠️ **아카이브 문서**: 2026-06-07 분리. 비대해진 원본 `docs/TASKS.md` 를 Phase 1~3 완료 이력 보존용으로 이관.
> 현행 task 추적은 `docs/TASKS.md`(신규), 부채 해소 로드맵은 `docs/progress/phase4-prep-debt-roadmap.md` 참조.
> 본 문서의 D-001~D-012 부채 상세도 여기에 보존된다(신규 TASKS.md 는 요약만 유지).
>
...
| D-008 | 리뷰 종합 (2026-04-10) | Monitoring | **Grafana datasource UID 하드코딩**. 모든 대시보드/알림에서 `"uid": "prometheus"`. Helm 기본값과 일치하는 한 문제 없으나 Helm 업그레이드 시 변경 가능성 | 양 환경 (`k8s/monitoring/{minikube,gke}/values-prometheus.yml`) 에 `grafana.sidecar.datasources.uid: prometheus` 명시 pin 추가. 차트 기본값 변경에도 dashboard/alert (`uid: prometheus`) 회귀 차단. `helm template` 검증으로 datasource ConfigMap 에 리터럴 `uid: prometheus` 렌더링 확인 | ~~낮음~~ **해결됨** |
...
| D-012 | Phase 4 진입 전 버킷 1 (2026-06-08) | CI / Deliverable | **CI 가 품질 게이트가 아니다** — PR Docker build·runtime smoke 부재, `main` branch protection 미설정, Kustomize namespace 누출 lint 부재 | chore/d012-ci-quality-gates (2026-06-08) — PR 경로에 Docker image build gate 추가(`docker/build-push-action@v5`, `push:false`, `load:true`, `cache-from:type=gha`, `peekcart-ci:${{ github.sha }}`), `scripts/docker-health-smoke.sh` 로 MySQL/Redis/Kafka + 앱 컨테이너 기동 후 `/actuator/health` 200 검증, `scripts/kustomize-namespace-lint.sh` 로 minikube/gke overlay namespaced 리소스 namespace 누락·불일치 차단, GitHub `main` branch protection 에 required status check `build` + strict mode 적용. PR cache write, PR review 강제, admin enforcement 는 범위 밖으로 미적용 | ~~높음~~ **해결됨** |
...
| 2026-05-07 | D-008 해결 (Grafana datasource UID 명시 pin) | `k8s/monitoring/{minikube,gke}/values-prometheus.yml` 양쪽에 `grafana.sidecar.datasources.uid: prometheus` 명시 추가. 차트 기본값과 동일하나 명시 선언으로 kube-prometheus-stack 업그레이드 시 기본값 변경에 따른 dashboard 22건 (`api-jvm` 7 + `kafka-lag` 3 + `pod-resources` 4) + alert 4 rule (`grafana-alerts.yml` 의 4 PromQL queries 8 datasourceUid 참조) 회귀 차단. 검증: `helm template` 으로 양 환경 모두 datasource ConfigMap 에 리터럴 `uid: prometheus` 렌더링 확인. 코드/dashboard JSON/alert YAML 변경 0건 (SSOT 는 values 한 군데로 단일화). 브랜치 `fix/d008-grafana-datasource-uid-pin`. PR: https://github.com/Kimgyuilli/PeakCart/pull/34 |
| 2026-06-07 | Tier A 즉시 정정 (Phase 4 진입 전 버킷 1) | D- 승격 없이 폐기하는 명백한 오류 5건 정정. L-018: ADR-0004/GKE README/loadtest 리포트 템플릿 cleanup 안내를 `bash loadtest/cleanup.sh` 단일 진입점으로 통일. L-021: `verify-concurrency.sql` A쿼리 판매량 집계가 취소/결제실패 주문 아이템을 세지 않도록 `o.id IS NOT NULL` 조건부 SUM 적용. L-022: `REPORT.md` CPU `400%` 해석을 request 500m 대비 2 vCPU(Pod limit 2000m 도달) 로 정정. L-016(b): GKE deployment patch 의 `imagePullPolicy` 주석을 `:latest` 기본값 `Always` 기준으로 정정. L-020(1): Kafka lag dashboard legend 를 실제 라벨 `client_id` 기반으로 변경. `TASKS.md` Tier A 상태 `✅` 완료 표기. 검증: `git diff --check HEAD~6..HEAD`, `jq empty k8s/monitoring/shared/kafka-lag-dashboard.json`, 낡은 문구 검색 통과. 브랜치 `chore/tier-a-immediate-fixes`. PR 미생성 |
| 2026-06-08 | D-012 해결 (CI 품질 게이트) | L-017: `scripts/kustomize-namespace-lint.sh` 신규 추가, `kubectl kustomize` 산출물에서 minikube/gke overlay namespaced 리소스가 `peekcart` namespace 를 직접 선언하는지 PyYAML 로 검증. L-015: PR Docker build gate 추가 + `scripts/docker-health-smoke.sh` 로 빌드 이미지를 실제 앱 컨테이너로 띄워 MySQL/Redis/Kafka compose 인프라 연결 및 `/actuator/health` 200 확인. `load:true` 는 smoke step 이 로컬 image tag 를 소비하므로 명시 유지. L-014: GitHub `main` branch protection 적용 (`strict=true`, required status check `build`, force push/delete 금지). 검증: namespace lint positive/negative, `docker build -t peakcart-ci:local .`, `bash scripts/docker-health-smoke.sh peakcart-ci:local`, branch protection API 재조회. 브랜치 `chore/d012-ci-quality-gates`. PR 미생성 |
