...
> **어떻게 자라나**: `/work`(또는 `/ship`)에서 구현이 계획과 **크게 달랐던** 항목이 나오면, 그 원인을 `Bn` 한 줄로 여기에 append 한다. 이게 compound 루프의 capture 단계다.
> **가지치기**: 자동 검사(테스트/린트)로 승격된 항목은 본 목록에서 제거하고 "승격됨"으로 표기. 본 목록은 *아직 자동화 못 한 계획 규율*만 담는다(비대화 방지).
