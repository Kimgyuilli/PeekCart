...
## 2026-09-22 — PR 제목 규약 위반과 lint 공백
- PR [#131](https://github.com/Kimgyuilli/PeekCart/pull/131) 제목이 `(D-029)` 를 달고 나갔다.
  `writing.md` §2 위반이고 #122~#130 아홉 개는 전부 지키고 있었다. 사용자가 지적했다.
- **원인은 검사 공백이다.** `writing-lint.sh` 에 제목 모드가 없었다. `--commits` 가
  커밋 제목에서 같은 위반을 경고로 잡아 고쳤으나, `gh pr create --title` 은 lint 경로가
  아예 없어 **고친 직후 같은 자리에서 재발**했다. `/ship` 이 커밋과 본문은 lint 하면서
  자기가 만드는 제목은 lint 하지 않는 비대칭이다.
- **두 번째 결함**: 추적 태그 정규식이 `[A-Z]{2,}-\d+` 여서 이 프로젝트의 부채 ID
  `D-029` 를 **한 번도 잡지 못했다**(대문자 한 글자 접두사). `ADR-0027`·`#131` 만 걸렸다.
  즉 커밋 모드의 경고조차 D-NNN 에는 안 돌고 있었다.
- 처분: 제목 수정(`gh pr edit`) + `--title` 모드 추가 + 정규식 수정 + self-test 7건 +
  `/ship` Step 4-1/7 삽입 + `writing.md` §6 표에 PR 제목 열. 제목 모드의 추적 태그는
  **오류**로 뒀다 (경고로 두면 넘어가고 실제로 넘어갔다. 볼드 밀도와 같은 판단).
- 회귀 확인: 과거 PR 제목 40건 전수 검사에서 위반 0건. #131 만의 일회성 누출이었다.
- compound capture 3-질문: ① 반복되나 Yes ② 재도출 어렵나 No ③ 자동화 가능 **Yes**
  → **prose 저장 안 함. 검사기로 전환 완료.**
