...
## 2026-09-24 — V-7 CI 실측 (완료)

- run 35976903613 전건 통과. 셔플 시드 1790239431744
- `:order-service:test` 975초 → 224초. 잡 벽시계 18.6분 → 5.2분. **V-7 통과**
- run5 이상치 미재현. 로컬 자원 고갈 가설이 남고 간헐 결함 증거는 없다
- **예측 실패 기록**: ADR-0028 의 "CI 19분 → 약 16분" 이 빗나갔다. 실제로는 19분 03초에서
  20분 49초로 **늘었다**. test 가 이미 임계경로 밖(D-031)이라 전체가 줄지 않는 것은 구조상
  당연한데, ADR 작성 시점에 그 계산을 하지 않고 test 단축분을 전체에 그대로 대입했다.
  계획서 §명제는 "CI 기여는 19분에서 약 16분에 그친다" 로 이미 낮춰 적었으나 그것도 낙관이었다
- 이번 run 의 images 1.9분 → 3.4분은 새 브랜치의 gha 캐시 콜드다(D-034 ref 격리와 동일).
  이 축을 빼도 전체가 16분이 되지는 않는다
- 부수 확증: e2e 16.1분 단독 병목. D-033 재개 조건 충족
