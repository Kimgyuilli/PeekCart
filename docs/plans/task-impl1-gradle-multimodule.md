...
- [ ] **N5.** (P10) notification 전속 global 이동: `idempotency/*` 복제(ProcessedEvent, 소비자 멱등성). **KafkaConfig 경계(ADR-0011 §D2 정합)**: producer/consumer factory 는 **`:common` 유지**, notification-service 에는 **listener/container factory·error-handler 등 서비스별 소비 배선만** 둔다. `kafkaErrorHandler`→SlackPort 는 N1 으로 `:common` SlackPort 를 주입하므로 root·notification 이 **각자 자기 KafkaConfig 에서 자기 DLQ/error handler 만** 배선(공유 factory 는 :common). **thin SecurityConfig**(common-auth `JwtSecurityConfigurer` + notification PUBLIC_URLS: swagger/actuator). `IntegrationTestConfig`(SlackPort 의존 → 이제 `:common`) notification test 로.
...
- [ ] **N8.** (P13) testFixtures 배선 + notification-service Testcontainers 통합 + `ObservabilityMetricsIntegrationTest`(application= 태그/`_bucket`/prometheus 200/health permitAll/exposure) + **보안 negative**(미인증 NotificationController 401/403 · blacklist hit·fail-closed·miss 회귀, 게이트 g) + **cross-module JWT**(root signer↔notification verifier, 게이트 h) + **SecurityFilterChain 1개·JwtFilter 1회**(게이트 j).
  ...
  - **root 부팅 스모크 (3차 리뷰 #2, 게이트 b 직결)**: root `@SpringBootTest` 컨텍스트 로드 스모크 — `SlackNotificationClient` 의 `:common` 이동 + root `slack.webhook.url` + `KafkaConfig` 의 SlackPort 주입 조합이 **한 번에** 부팅됨을 검증(게이트 b 를 N8 에 실효 매핑). notification `bootJar`(게이트 c)는 N3 에 매핑.
...
- [ ] **P10.** 서비스 전속 global 분배(§D2): User=`SecurityConfig`(인증부+actuator S4)+`auth`(+`WebMvcConfig`,`OpenApiConfig`)/`jwt`/`security` · Order/Payment=`outbox`+`ShedLockConfig`+`KafkaConfig`(토픽/DLQ 빈 + PR1 이연 팩토리) · Product=`CacheConfig`(S7) · Order/Payment/Notification=`idempotency`(ProcessedEvent 복제) · ~~Notification=`port.SlackPort`~~ → **SlackPort 는 `:common`**(N1/ADR-0011 §D2 Update Log 정정). PR1 이연분(WebMvcConfig/OpenApiConfig/KafkaConfig 팩토리/SecurityConfig S4/IntegrationTestConfig/fixtures) 포함.
