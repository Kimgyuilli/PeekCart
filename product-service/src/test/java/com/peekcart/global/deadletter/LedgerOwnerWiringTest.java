 ...
 */
@DisplayName("원장 소유자 배선 (product-service)")
class LedgerOwnerWiringTest {
    ...
    @Test
    @DisplayName("컨테이너에 올린 LedgerOwner 는 PRODUCT 다 — DeadLetterConsumer.SELF 와 같아야 한다")
    void ledgerOwnerIsThisService() {
        runner.run(context -> {
            ...
            assertThat(owner.service()).isEqualTo(PeekcartService.PRODUCT);
            assertThat(owner.prefix()).isEqualTo("product");
            assertThat(owner.owns("product")).isTrue();
